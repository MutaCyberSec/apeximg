    ...
    if request.method == "POST":
        if "img" in request.files:
            ...
            if allowed_file(filename):
                ...
                coll.insert_one({"filename" : filename , "image" : uploa , "name" : name})
...
def view_pic():
    ...
    decodeit = open(new_name + ".jpg", 'wb')
    decodeit.write(base64.b64decode((bina)))
    decodeit.close()
    
    
    return render_template("view_pic.html" , name = name , bina = bina , ima = decodeit)
